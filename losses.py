...
__all__ = [
    ...
    "useful_tree_predicate",
    "ClippedLeastSquaresError",
    "ClippedLSEPredicate",
    "ClippedBinomialDeviance",
    "ClippedMultinomialDeviance",
    "RootExpQLeastSquaresError",
    "RootMedianLeastSquaresError",
    "DP_quasi_rMSE",
]
...
class ClippedLeastSquaresError(LeastSquaresError):  # type: ignore
    ...
    def __call__(
        ...
        if sample_weight is None:
            return cast(
                float, np.mean(np.clip((y - raw_predictions.ravel()) ** 2, -c, c))
            )
        else:
            raise NotImplementedError(
                "Clipping is not implemented if argument `sample_weight`" "is not None."
            )

    def __repr__(self) -> str:
        return ("ClippedLeastSquaresError(" "clipping_bound={})").format(
            self.clipping_bound
        )
...
class ClippedLSEPredicate:
    ...
    def __repr__(self) -> str:
        _repr = (
            "ClippedLSEPredicate(privacy_budget={},clipping_bound={}," "random_seed={})"
...
class RootExpQLeastSquaresError(LeastSquaresError):  # type: ignore
    ...
    def _utilities(self, x: np.ndarray, q: float) -> np.ndarray:
        ...
        utilities = cast(
            np.ndarray,
            np.where(
                _x < median_position, _x + 1 - median_position, median_position - _x
            ),
        )
        return utilities
    ...
    def _probabilities(
        self, utilities: np.ndarray, bin_sizes: np.ndarray, privacy_budget: float
    ) -> np.ndarray:
...
class RootMedianLeastSquaresError(LeastSquaresError):  # type: ignore
    def __call__(
        ...
        if sample_weight is None:
            # Note that sqrt(median(x_1 ** 2, x_2 ** 2, ..., x_n ** 2))
            #           = median(|x_1|, |x_2|, ..., |x_n|)
            return cast(float, np.median(np.abs(y - raw_predictions.ravel())))
        else:
            raise NotImplementedError(
                "RMedLSE is not implemented if argument `sample_weight` is " "not None."
            )
...
class RootQuantileLeastSquaresError(LeastSquaresError):  # type: ignore
    ...
    def __call__(
        ...
        if sample_weight is None:
            ...
            return cast(float, np.quantile(np.abs(y - raw_predictions.ravel()), self.q))
        else:
            raise NotImplementedError(
...
class DP_quasi_rMSE(LeastSquaresError):  # type: ignore
    def __init__(
        self, privacy_budget: float, beta: float, L: float, U: float, m: int, seed: int
    ) -> None:
        super().__init__()
        self.privacy_budget = privacy_budget
        self.beta = beta
    ...
    def __call__(
        ...
        if sample_weight is None:
            ...
            s = smooth_sensitivity(
                trimmed_sample, 0.0, self.beta, self.L, self.U, "std"
            )
    ...
    def __repr__(self) -> str:
        ...
        return template.format(self.privacy_budget, self.beta, self.L, self.U, self.m)


class DP_rMSE(LeastSquaresError):  # type: ignore
    def __init__(
        self, privacy_budget: float, U: float, seed: Optional[int] = None,
    ) -> None:
        super().__init__()
        self.privacy_budget = privacy_budget
        self.U = U
    ...
    def __call__(
        ...
        if sample_weight is None:
            ...
            logger.debug("max(abs(diffs)): %f", np.max(np.abs(differences)))
            loss = dp_rMS_cauchy(differences, self.privacy_budget, self.U, self.seed)
            if loss < 0.0:
                logger.warn(
            ...
            raise NotImplementedError(
                "DP_rMSE is not implemented if argument `sample_weight`" "is not None."
            )
